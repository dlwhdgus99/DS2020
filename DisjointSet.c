...
void Insert(BTTree *Tree, DATATYPE Data, DATATYPE Parent)
{
    ...
    if(Tree->Head->next == NULL) Tree->Tail = Tree->Head;
    Tree->Tail->next = node;
    Tree->Tail = node;
    Tree->NumofData++;
}
...
DATATYPE Search(BTTree *Tree, DATATYPE *Data)
{
    ...
    if(PosHead(Tree, &temp)){
        if(temp == *Data) return Tree->Curr->Parent;
        while(PosNext(Tree, &temp)){
            if(temp == *Data) return Tree->Curr->Parent;
        }
    }
    return NULL;
...
DATATYPE MakeAddressMapWithSubTree(BTTree *Tree, BTNode *Node)
{
    ...
    else if(Node->Right == NULL && Node->Left == NULL) return Node->Data;

    MakeAddressMapWithSubTree(Tree, Node->Left);

    if(Node->Left == NULL) Insert(Tree, Node->Right->Data, Node->Data);
    else if(Node->Right == NULL) Insert(Tree, Node->Left->Data, Node->Data);
    else{
        Insert(Tree, Node->Left->Data, Node->Data);
        Insert(Tree, Node->Right->Data, Node->Data);
    }

    MakeAddressMapWithSubTree(Tree, Node->Right);
...
DATATYPE Find(BTTree *Tree, DATATYPE *Data)
{
    if(Search(Tree,Data) <= 0) return *Data;
    else return Find(Tree, &Tree->Curr->Parent);
}

int Union(BTTree *Tree1, BTTree *Tree2)
{
    int HeightOfTree1 = 0 , HeightOfTree2 = 0;
    DATATYPE temp;
    DATATYPE tempHeight = 0;

    if(PosHead(Tree1, &temp)){
        tempHeight = Search(Tree1, &temp);
        if(tempHeight <= 0) HeightOfTree1 = tempHeight;
        while(PosNext(Tree1, &temp)){
            tempHeight = Search(Tree1, &temp);
            if(tempHeight <= 0) HeightOfTree1 = tempHeight;
        }
    }

    if(PosHead(Tree2, &temp)){
        tempHeight = Search(Tree2, &temp);
        if(tempHeight <= 0) HeightOfTree2 = tempHeight;
        while(PosNext(Tree2, &temp)){
            tempHeight = Search(Tree2, &temp);
            if(tempHeight <= 0) HeightOfTree2 = tempHeight;
        }
    }
    ...
    if(HeightOfTree1 < HeightOfTree2){
        if(PosHead(Tree2, &temp)) {
            InsertBST(&Tree1->Root, Tree2->Curr->Data);
            while (PosNext(Tree2, &temp)) {
                InsertBST(&Tree1->Root, Tree2->Curr->Data);
            }
        }
        Tree1->NumofData += Tree2->NumofData;
        ...
        if(PosHead(Tree1, &temp)) {
            InsertBST(&Tree2->Root, Tree1->Curr->Data);
            while (PosNext(Tree1, &temp)) {
                InsertBST(&Tree2->Root, Tree1->Curr->Data);
            }
        }
        Tree2->NumofData += Tree1->NumofData;
